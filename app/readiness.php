...
function estab_readiness_schema_query(): string
{
    return "SELECT "
        ...
        . "AND is_nullable = 'NO' "
        . "AND HEX(column_default) = '2727' AND extra = '' "
        . "AND ((column_name = '11_rufnummer' "
        . "AND data_type = 'varchar' AND column_type = 'varchar(128)' "
        . "AND character_maximum_length = 128 "
        . "AND column_comment = "
        . "'estab:migration:98:message-counterparty-number:v1') OR "
        . "(column_name = '12_betreff' "
        . "AND data_type = 'varchar' AND column_type = 'varchar(255)' "
        . "AND character_maximum_length = 255 "
        . "AND column_comment = "
        . "'estab:migration:98:message-subject:v1'))) = 2 "
        . "AND ((SELECT CONCAT(GROUP_CONCAT(column_name "
        . "ORDER BY ordinal_position SEPARATOR ','),':',"
        . "MAX(ordinal_position) - MIN(ordinal_position)) "
        . "FROM information_schema.columns "
        . "WHERE table_schema = DATABASE() "
        . "AND table_name = 'nv_nachrichten' AND column_name IN ("
        . "'10_anschrift','11_rufnummer','11_gesprnotiz',"
        . "'12_betreff','12_anhang')) = "
        . "'10_anschrift,11_rufnummer,11_gesprnotiz,"
        . "12_betreff,12_anhang:4')) "
        . "AND ((SELECT COUNT(*) FROM information_schema.statistics "
        . "WHERE table_schema = DATABASE() "
        . "AND table_name = 'nv_nachrichten' "
        ...
        . "AND index_type = 'FULLTEXT' AND non_unique = 1 "
        . "AND sub_part IS NULL) = 7) "
        . "AND ((SELECT GROUP_CONCAT(column_name "
        ...
        . "AND is_nullable = 'YES' AND extra = '' AND column_comment = "
        ...
        . "AND ordinal_position = (SELECT ordinal_position + 1 "
        . "FROM information_schema.columns "
        . "WHERE table_schema = DATABASE() AND table_name = 'nv_einsaetze' "
        ...
        . "AND ordinal_position = (SELECT ordinal_position + 2 "
        . "FROM information_schema.columns "
        . "WHERE table_schema = DATABASE() AND table_name = 'nv_einsaetze' "
        . "AND column_name = 'organisation') "
        . "AND extra = '' AND column_comment = "
        . "'estab:migration:97:incident-command-post-lock:v1') = 1) "
        . "AND ((SELECT COUNT(*) FROM nv_einsaetze "
        . "WHERE fuehrungsstellenname IS NOT NULL AND ("
        . "BINARY fuehrungsstellenname "
        . "<> BINARY TRIM(fuehrungsstellenname) "
        . "OR CHAR_LENGTH(fuehrungsstellenname) = 0 "
        . "OR fuehrungsstellenname REGEXP '^[[:space:]]|[[:space:]]$' "
        . "OR fuehrungsstellenname NOT REGEXP '[^[:space:]]' "
        . "OR fuehrungsstellenname REGEXP '\\\\p{C}')) = 0) "
        . "AND ((SELECT COUNT(*) FROM nv_einsaetze "
        . "WHERE fuehrungsstellenname_gesperrt NOT IN (0,1) "
        . "OR (fuehrungsstellenname_gesperrt = 1 "
        . "AND fuehrungsstellenname IS NULL)) = 0) "
        . "AND ((SELECT COUNT(*) FROM information_schema.triggers "
        . "WHERE trigger_schema = DATABASE() AND trigger_name IN ("
        . "'estab_command_post_incident_insert',"
        . "'estab_command_post_incident_update')) = 2) "
        ...
        . "AND ((SELECT COUNT(*) FROM estab_schema_migrations) = 15) "
        ...
        . "'99-message-list-search.sql','100-session-presence.sql') "
        . "AND state = 'applied' "
