...
| Fresh-Baseline und Upgrade eines Legacy-Schemas | `tests/integration/schema_migrator.sh` | Ein checksumgebundener Abbruch wird idempotent bis zur aktuellen Migrationsfolge fortgesetzt; unprotokollierte Teilbestände, Duplikate, fremde Objektkollisionen und Checksum-Manipulation blockieren ohne stilles Umschreiben. Migrationen 110 bis 117 bleiben als historische Entwicklung checksumstabil. Migration 118 ergänzt die kollisionsgeprüfte Tabelle `nv_benutzer_zusatzfunktionen` und ersetzt den abschließenden ETB-/TTB-, Fernmeldeplan- und Meldertriggervertrag: `STRICT` verlangt aktive persönlich angenommene Dienstbesetzungen und manuelle Buchprovenienz, `LOOSE` eine feste oder explizite Zusatzfunktion ohne formale Dienstschicht. Veröffentlichte ältere Migrationen und Fachdaten werden nicht umgeschrieben. |
...
118 endete mit Exitcode 0 und `CI integration: OK`. Er bestätigte alle 24
Migrationen, 42 Schema-Prüfungen, 69 Einsatz-, 52 DV-Evidenz- und 236
...
Containeridentität ausgeführte statische Suite endete ebenfalls mit Exitcode 0
und lintete 277 aktive PHP-Dateien. Darin bestanden insbesondere 99 Assertions
zum Berechtigungsmodus, 30 zum exakten LOOSE-Anhang-Scope und 249 zum
Single-Dispatch-Vertrag.
...
Abnahmen für Fernmeldeplan-Versionierung, Meldungsüberschriften und
Nachrichtenvorschläge. Der S6-Browsernachweis umfasst den vollständigen Klon,
