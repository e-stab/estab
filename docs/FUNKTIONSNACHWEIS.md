...
| Anmeldung und Rollenbindung | `tests/php/auth_security.php`, `tests/php/workflow_security.php`, `tests/integration/http_surface_http.sh`, `tests/integration/http_smoke.sh` | Startseitenbutton, Kontenlisten-Auswahl und getrennte, voranmelde-CSRF-gebundene Konto-Flows funktionieren; Bestandslogin erzeugt kein Konto, Neuanlage übernimmt weder vorhandenes Kürzel noch Passwort-Hash; der explizit freigeschaltete historische Zwei-Kennwort-Pfad bleibt für Same-Site-Clients kompatibel und weist Cross-Site ab; aktive Konten können Funktion/Rolle nicht per Request wechseln, inaktive Konten erst nach Logout; zweite Sitzung, Session-Grenze und sechsstellige Kürzel bleiben korrekt |
