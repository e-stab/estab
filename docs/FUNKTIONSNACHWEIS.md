...
| --- | --- | --- |
| Einsatzbezogener Berechtigungsmodus | `tests/php/permission_mode_security.php`, `tests/php/permission_mode_attachment_scope_security.php`, `tests/php/incident_ui_security.php`, `tests/integration/schema_migrator.sh`, `tests/integration/incident_domain.php`, `tests/integration/dv_operations.php`, `tests/integration/message_concurrency.php`, `tests/integration/message_workflow_http.sh` | Bestand und Neuanlage bleiben standardmäßig `STRICT`; Modus und Warnung sind im Einsatzstatus sichtbar. Anlegen, Aktivieren oder Umstellen auf `LOOSE` verlangt die ausdrückliche Bestätigung, erwarteten Altmodus und unveränderte globale Revision; jeder echte Wechsel wird Vorher/Nachher auditiert. `STRICT` bewahrt die bisherigen funktions-/rollenbezogenen Schreibrechte. `LOOSE` erlaubt funktionsübergreifendes Schreiben ausschließlich in den dafür vorgesehenen operativen Schreibwegen von Nachrichtenworkflow, ETB/TBB, S6-Planung und Melderlauf sowie ausschließlich mit konkretem aktivem und ungesperrtem Konto. Es lockert weder Authentisierung, Zugangsschichtentzug, aktiven offenen Einsatz/Scoping, allgemeine Leserechte, rollenstrenge Übersichten/Kategorien-/Adminrechte, CSRF/Validierung, Richtung, Workflowzustände, Sperrinhaber, Integrität, Ereignisketten, Append-only noch Retention. Eine explizit gewählte Schreibstufe erhält nur die dafür erforderliche Objektsicht; zurückgewiesene Ausgänge dürfen funktionsübergreifend übernommen werden, während Evidenz alte und neue Verantwortung bewahrt. Unmarkierte Legacy-DML, kombinierte Einsatzänderungen und ein Modus-/Revisionswechsel während eines Writers scheitern für den Anwendungsweg fail-closed. Die connection-lokalen Marker sichern dabei Kohärenz, sind aber keine Privileggrenze: Ein SQL-Principal mit beliebigen Schreibrechten ist eine ausdrücklich vertrauenswürdige Betreibergrenze. |
...
| PHP-8.5-Laufzeit | `tests/static/run.sh` | alle aktiven PHP-Dateien einschließlich `app/permission_mode.php` linten; Kompatibilitäts- und Sicherheitsassertionen sind grün |
...
| Fresh-Baseline und Upgrade eines Legacy-Schemas | `tests/integration/schema_migrator.sh` | Ein checksumgebundener Abbruch wird idempotent bis zu allen 22 Migrationen fortgesetzt; unprotokollierte Teilbestände, Duplikate, fremde Objektkollisionen und Checksum-Manipulation blockieren ohne stilles Umschreiben. Migration 110 bindet lokale ETB-/TBB-Nummern und Append-only-Grenzen, Migration 111 ergänzt nullable historische Schicht-/Schreiberprovenienz. Migration 112 führt optionale Zugangsschichten ein und ersetzt den damaligen abschließenden Triggervertrag: aktiver Einsatz und feste fachlich passende Kontofunktion sind Pflicht, eine aktive Schicht oder Besetzungs-ID nicht. Migration 113 ergänzt die revisionsgesicherte prospektive Kennwortrichtlinie mit unverändertem Default 12 und verändert keinen Bestands-Hash. Migration 114 ergänzt checksumstabil die revisionsgesicherte dauerhafte oder DB-UTC-befristete Selbstregistrierungsfreigabe. Nur ein im selben neuen Baseline-Lauf angelegter, mit der 114-SHA gebundener Marker stellt ihre pristine Startzeile atomar auf `DISABLED`; Retry und Zweitlauf sind idempotent, Marker-/Checksum-Manipulation blockiert, während echte Upgrades und frühere markerlose Fresh-Installationen kompatibel `ENVIRONMENT` behalten. Migration 115 setzt Bestand und Neuanlage auf `STRICT`, schützt Modusänderungen mit Guard-Triggern und ersetzt sechs Rollen-Trigger modebewusst, ohne Vorgängerdateien oder Fachdaten umzuschreiben. Migration 116 belegt ausschließlich eine vollständig leere globale Kategorienliste einmalig mit `Allgemein` und `EA1` bis `EA6`; vorhandene Kataloge sowie spätere Änderungen und Löschungen bleiben unverändert. Veröffentlichte ältere Migrationen bleiben checksumstabil. |
...
| Stab-Nachricht und Vorrang | `tests/php/message_security.php`, `tests/php/message_priority_security.php`, `tests/php/official_message_form_ui_security.php`, `tests/php/read_authorization_security.php`, `tests/integration/http_smoke.sh`, `tests/browser/headless_ui.py` | Formular öffnet mit gültigem, fachlich berechtigtem Festfunktionskonto und aktivem Einsatz auch ohne aktive Schicht; rohe UTF-8-Nachricht wird gespeichert und wieder gelesen; Quotes, `&`, `<script>` und SQL-ähnlicher Text bleiben Daten; einsatzfremde sowie nach Richtung, Status oder Sperrinhaber unzulässige Objekte werden immer und unpassende Kontofunktionen im strengen Modus abgewiesen; der lockere Modus lässt für eine ausdrücklich gewählte Schreibstufe die funktionsbezogene Prüfung und nötige Workflow-Objektsicht entfallen. Vorrang wird zentral als keine, Sofort, Blitz oder Staatsnot dargestellt und streng validiert. Sofort, Blitz und Staatsnot liegen gemeinsam im einzigen Vorrangfeld des Nachrichtenvordrucks; es gibt weder eine zweite Vorrangauswahl noch ein paralleles sichtbares Erreichbarkeitsfeld außerhalb des amtlichen Blatts. |
| Rollenübergreifender Nachrichtenlauf | `tests/php/config_security.php`, `tests/php/official_message_form_ui_security.php`, `tests/php/message_form_fields_security.php`, `tests/php/ldf_validation_security.php`, `tests/php/ldf_ui_flow_security.php`, `tests/php/read_authorization_security.php`, `tests/php/message_suggestion_security.php`, `tests/integration/message_suggestions.php`, `tests/integration/message_workflow_http.sh`, `tests/browser/headless_ui.py`, orchestriert durch `tests/integration/ci.sh` | Getrennte Festfunktionskonten für LdF, Fernmelder, Si, Stab und FB durchlaufen den verbindlichen Ablauf: Eingang `Fernmelder → LdF → Si → abgeschlossen`, Ausgang `Verfasser → Si → LdF → Fernmelder → abgeschlossen`. Bedieneridentität, Funktion und Rolle stammen aus dem Konto; ein Browser kann sie nicht durch eine Hutauswahl oder Formularwerte ersetzen. Rufnamen-/Absendervorschläge, Statusfolge, Sperrbesitz und Einsatzgrenze werden serverseitig geprüft. Ein aktiver Einsatz ist zwingend, eine aktive Schicht nicht. |
...
| ETB/TBB | `tests/php/logbook_security.php`, `tests/php/generated_form_security.php`, `tests/php/read_authorization_security.php`, `tests/php/schema_migration_contract.php`, `tests/integration/schema_migrator.sh`, `tests/integration/logbooks_http.sh`, `tests/integration/dv_operations.php`, `tests/integration/dv_evidence.php`, `tests/integration/incident_export.php`, Restore-Zweig in `tests/integration/http_smoke.sh` | Beide Bücher sind einsatzgebunden und append-only. Im strengen Modus schreiben Festfunktionskonten `ETB/Stab` oder `S2/Stab` mit `EINSATZTAGEBUCH` das ETB und `Fernmelder` mit `BEFOERDERUNG` das TTB; im lockeren Modus entfällt ausschließlich diese Funktion-/Rollenbedingung. Anwendung und Insert-Trigger verlangen in beiden Modi aktiven Einsatz und konkretes ungesperrtes Konto, in `STRICT` zusätzlich passende feste Funktion/Rolle, aber keine aktive Schicht oder Besetzungs-ID. Neue Zeilen dürfen die Legacy-Schicht-/Schreiberprovenienz `NULL` lassen; belegte Altwerte bleiben erhalten. Der Einsatz-Insert erzeugt exakt die Köpfe `ETB:1` und `TTB:1`; lokale Nummern bleiben unter Konkurrenz eindeutig. Ein Einsatzabschluss ist auch ohne frühere Schichteröffnung möglich. Korrekturen sind neue, direkt referenzierte Zeilen; Mindestaufbewahrung zehn Jahre. |
...
## Stand des Abschlusslaufs vom 1. August 2026

Der vollständige Lauf von `tests/integration/ci.sh` einschließlich Migration
115 endete mit Exitcode 0 und `CI integration: OK`. Er bestätigte 42
Schema-Prüfungen, 55 Einsatz-, 51 DV-Evidenz-, 145 DV-Operations-Assertions
mit 80 Ereignissen, 25 Zugangsschicht-, 98 Benutzerverwaltungs-, 66
MariaDB-Kennwortrichtlinien-, 31 Selbstregistrierungs-Datenbank- plus 28
Handler-, 59 Zuordnungs-, 35 Einsatz-PDF-, 11 Anhangdarstellungs-, 29
Nachrichtenvorschlags- und 145 Nachrichtensuch-Assertions. HTTP-Surface,
Selbstregistrierung, Auth-Smoke, Logbücher, Kategorien, Nachrichtenworkflow,
Administrationsworkflow sowie Backup und Restore endeten ebenfalls
erfolgreich.

Die danach vollständig ausgeführte statische Suite endete ebenfalls mit
Exitcode 0 und lintete 269 aktive PHP-Dateien. Darin bestanden insbesondere
80 Assertions zum Berechtigungsmodus, 29 zum exakten LOOSE-Anhang-Scope und
222 zum Single-Dispatch-Vertrag.

Nachweisgrenze für den Berechtigungsmodus: Im finalen Lauf stand kein
Browser-Steuerwerkzeug zur Verfügung. Deshalb sind die Umschaltung zwischen
`STRICT` und `LOOSE`, die sichtbare Moduswarnung und ein
Cross-Rollen-Schreibweg nicht im echten Browser nachgewiesen. Die dafür
vorhandenen statischen, MariaDB- und authentifizierten HTTP-Nachweise waren
erfolgreich; die Bedienung im Browser bleibt bis zu einem eigenen
automatisierten Szenario Teil der verpflichtenden manuellen Fachabnahme.

Ergänzend bindet der aktuelle ETB-/TBB-Nachweis im strengen Modus die festen
Schreibrechte: `ETB/Stab` und `S2/Stab` schreiben ETB; die Funktion
`Fernmelder` schreibt TTB. Ein aktiver Einsatz und konkretes ungesperrtes Konto
sind in beiden Modi Pflicht; eine aktive Schicht, persönliche Annahme oder
Hutauswahl nicht. Migration 112 installierte den strengen Triggervertrag,
Migration 115 ergänzt den modebewussten lockeren Pfad, ohne die
...
optional. Der Dossierdialog bietet ETB/TBB als Gesamtbuch oder für historisch
belegte formale Dienstschichten an. Dieser Legacy-Filter umfasst keine
Zugangsschicht und keine neue Zeile mit `NULL`-Provenienz. Er filtert
...
aufgelösten Umfang samt historischen Schichtmetadaten.

Neue ETB-/TTB-Zeilen dürfen die Legacy-Felder für Dienstschicht und schreibende
Dienstbesetzung `NULL` lassen; vorhandene belegte Werte bleiben erhalten. Eine
...
- einen offenen Einsatz im Browser zunächst in `STRICT` betreiben und einen
  fachfremden Schreibversuch abweisen; anschließend den Modus in der
  Administration mit Warnungsbestätigung auf `LOOSE` stellen, die sichtbare
  Warnung prüfen und genau denselben ausdrücklich angebotenen Schreibschritt
  zulassen. Anmeldung, Kontosperre, Einsatzbindung, CSRF, Objektstatus,
  Sperrinhaber, Audit und Integritätsgrenzen müssen weiter greifen. Danach den
  Einsatz wieder auf `STRICT` stellen und den fachfremden Versuch erneut
  abweisen,
- einen verknüpften und einen freien Anhang über Liste, Vorschau, Download,
  ...
  unberechtigtem Festfunktionskonto prüfen,
...
- Hinweistöne für Fernmelder, Si und Stab/FB in jedem vorgesehenen Browser
  ...
  nicht,
- optionale Zugangsschichten anlegen, ein Konto mehrfach zuordnen, OR-Semantik,
...
- vollständigen Pflichtkopf anlegen; lokale Nummer 1 ohne Schichtvoraussetzung,
  ETB-Schreibrecht für `ETB/Stab` und `S2/Stab` sowie TTB-Schreibrecht für
  die Funktion `Fernmelder` prüfen; andere feste Funktionen dürfen nicht schreiben,
...
- nullable Legacy-Schicht-/Schreiberprovenienz, Abschluss-Preflight ohne
  Schichtblocker, automatische Abschlusszeilen, formalen Einsatzabschluss,
