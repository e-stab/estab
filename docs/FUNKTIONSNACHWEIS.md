...
| Backup/Wiederherstellung | CI-Roundtrip und `docs/BACKUP-UND-WIEDERHERSTELLUNG.md` | der guardierte Lauf löscht Container und alle drei CI-Volumes, stellt sie leer neu bereit und weist danach Schema, Anmeldung/Konto, Nachricht, exakten Anhanginhalt, bytegleichen generierten PDF-Vordruck, vorhandene ETB-/TBB-Titel und -Einträge sowie genau den zuvor validierten Exportlauf mit unverändertem ZIP-SHA-256 nach |
| Pull-only OCI-/NAS-Deployment | `tests/php/registry_deployment_contract.php`, `tests/integration/registry_compose.sh`, `.github/workflows/publish-images.yml` | Registry-Compose enthält keinen Build und keinen Host-Schema-Mount, verlangt explizite unveränderliche Image-Referenzen und akzeptiert kein stilles `latest`; ein frisches Zweitprojekt startet ausschließlich die angeforderten Image-IDs, initialisiert das eingebettete Basisschema, verlangt Migrator Exit 0 und eine gesunde App und beweist die Entfernung seiner Container und Volumes vor dem grünen Ergebnis; der manuelle, global serialisierte Publish-Pfad ist Git-Tag-, Rechte-, Variablen- und Required-Reviewer-gebunden, verweigert Überschreiben, baut beide Kandidaten vor dem ersten Push und prüft deren amd64/arm64-Manifeste, SBOM, Provenance und Attestation |
