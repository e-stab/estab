...
| Rollenübergreifender Nachrichtenlauf | `tests/php/config_security.php`, `tests/integration/message_workflow_http.sh`, orchestriert durch `tests/integration/ci.sh` | fünf isolierte Konten für A/W, Si, S1, S2 und S3 werden über die echte Oberfläche angelegt; ein Eingang durchläuft `4 → 8`, ein Ausgang bei `ESTAB_REVIEW_OUTGOING_MESSAGES=false` direkt `2 → 8` und nach kontrollierter App-Neuerstellung bei `true` vollständig `2 → 4 → 8`; CSRF-, Sperr-, Status-, Abschluss-, Vordruck- und Listenverträge sowie die exakten Empfänger `S2_rt`, `S1_gn` und `S3_bl` werden in UI und MariaDB geprüft; boolescher Legacy-Fallback, Umgebungsoverride und ungültige Werte sind statisch belegt; der A/W-Zweitprüfpfad bietet Speichern/Abbrechen, lässt nur Quittierungszeichen, Empfängerfarben und Vermerk ändern, zeigt den ursprünglichen Quittierungszeitpunkt schreibgeschützt und erhält ihn selbst bei manipuliertem POST ebenso wie Felder 1–14, Transportstatus, Abschluss und den bereits erzeugten Vordruck |
